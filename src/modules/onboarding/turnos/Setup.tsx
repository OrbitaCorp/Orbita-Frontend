...
const PASOS_INTERNOS = ['Tus servicios', 'Tu negocio', 'Ubicación', 'Métodos de pago', 'Tu equipo']
...
function SkeletonStepUbicacion() {
  return (
    <div style={{ maxWidth: 540, margin: '0 auto' }}>
...
function SkeletonPaso({ paso }: { paso: number }) {
  ...
  if (paso === 2) return <SkeletonStepUbicacion  />
  if (paso === 3) return <SkeletonStepPagos      />
  ...
  return (
    <div style={{ maxWidth: 540, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        fontSize: 12, color: 'var(--color-body)', marginBottom: 20,
      }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Field label="Subdominio de tu negocio">
          <div style={{
            ...
            {estadoSub === 'checking' && (
              <span style={{ width: 16, height: 16, borderRadius: '50%', flexShrink: 0, marginRight: 12, border: '2px solid var(--color-primary)', borderTopColor: 'transparent', display: 'inline-block', animation: 'spin 600ms linear infinite' }} />
            )}
            {estadoSub === 'disponible' && (
              <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-success)', marginRight: 12, flexShrink: 0, whiteSpace: 'nowrap' }}>✓ Disponible</span>
            )}
            {estadoSub === 'ocupado' && (
              <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-error)', marginRight: 12, flexShrink: 0, whiteSpace: 'nowrap' }}>✗ No disponible</span>
            )}
          </div>
  ...
  return (
    <div style={{ maxWidth: 540, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--color-muted)', margin: 0 }}>
          Contanos si tenés un espacio físico donde atendés a tus clientes.
        </p>
      </div>
        ...
        {[
          {
            ...
            Icon: MapPin,
            label: 'Local físico',
            desc: 'Tengo un local, showroom, consultorio o espacio propio donde atiendo',
          },
          {
            key: 'online' as const,
            Icon: Globe,
            label: 'Solo online / A domicilio',
            desc: 'Trabajo de forma remota o viajo al domicilio del cliente',
          },
        ].map(({ key, Icon, label, desc }) => {
          const sel = tipoLocal === key
          return (
            <button
              key={key}
              onClick={() => setTipo(key)}
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--color-muted)', lineHeight: 1.5 }}>
              Tu perfil en Órbita va a mostrar que atendés a domicilio o de forma remota. Podés agregar una dirección después si cambia.
            </div>
          </div>
        </div>
      )}
...
export function TurnosSetup() {
  ...
  const puedeAvanzar =
    paso === 0 ? servicios.length > 0 :
    paso === 2 ? negocio.tipoLocal !== '' :
    true

  function avanzar() {
    if (paso < 4) { setCargandoPaso(true); setPaso(p => p + 1) }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
      ...
      {/* ── Inner step progress ── */}
      <div style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface)', padding: '0 28px', overflowX: 'auto', scrollbarWidth: 'none' }}>
        <div style={{ maxWidth: 720, margin: '0 auto', display: 'flex', alignItems: 'center', minWidth: 560 }}>
            ...
            return (
              <Fragment key={label}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '11px 0', flexShrink: 0, opacity: (done || current) ? 1 : 0.45 }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: current ? 600 : 500, whiteSpace: 'nowrap', color: current ? 'var(--color-text)' : done ? '#10B981' : 'var(--color-muted)', transition: 'color 300ms' }}>
                    {label}
                  </span>
                </div>
                {!isLast && (
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '36px 24px 160px' }}>
        {cargandoPaso
          ...
          : <>
              ...
              {paso === 2 && <StepUbicacion  negocio={negocio}    setNegocio={setNegocio}          />}
              {paso === 3 && <StepPagos     pagos={pagos}        setPagos={setPagos}              />}
              {paso === 4 && <StepEquipo    tamano={tamano}      setTamano={setTamano}            />}
            </>
        }
      </div>
