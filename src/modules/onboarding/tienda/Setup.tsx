...
const METODOS: { key: string; Icon: LucideIcon; label: string; desc: string }[] = [
  { key: 'efectivo',      Icon: Banknote,  label: 'Efectivo',         desc: 'Pagos en mano al momento de la entrega' },
...
const PASOS_INTERNOS  = ['Tipo de tienda', 'Tu negocio', 'Métodos de pago']
...
function Input({ value, onChange, placeholder, type = 'text', paddingLeft }: {
  value: string; onChange: (v: string) => void
  placeholder?: string; type?: string; paddingLeft?: number
}) {
  return (
    <input
      type={type}
      ...
      style={{ ...inputBase, ...(paddingLeft ? { paddingLeft } : {}) }}
    />
  )
}
...
function SkeletonStepTipo() {
  return (
    <div>
      <SkeletonTitle w1={270} w2={340} />
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(175px, 1fr))', gap: 10 }}>
        {Array.from({ length: 9 }).map((_, i) => (
          <div key={i} style={{ padding: '13px 13px 12px', borderRadius: 14, border: '1.5px solid var(--color-border)', background: 'var(--color-surface)' }}>
            ...
            <Skeleton width="85%" height={11} radius={4}  style={{ display: 'block', marginBottom: 2 }} />
            <Skeleton width="70%" height={11} radius={4}  style={{ display: 'block' }} />
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} style={{ padding: '20px 18px 18px', borderRadius: 14, border: '1.5px solid var(--color-border)', background: 'var(--color-surface)' }}>
            ...
            <Skeleton width="85%" height={12} radius={4}  style={{ display: 'block', marginBottom: 4  }} />
            <Skeleton width="70%" height={12} radius={4}  style={{ display: 'block' }} />
          </div>
        ))}
      </div>
    </div>
...
function SkeletonPaso({ paso }: { paso: number }) {
  if (paso === 0) return <SkeletonStepTipo    />
  if (paso === 1) return <SkeletonStepNegocio />
  return                  <SkeletonStepPagos  />
}

// ─── Step 0 — Tipo de tienda ──────────────────────────────────────────────────

function StepTipo({ subrubros, toggleSubrubro }: { subrubros: string[]; toggleSubrubro: (k: string) => void }) {
  return (
    <div>
      <div style={{ textAlign: 'center', marginBottom: 28 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(175px, 1fr))', gap: 10 }}>
        {SUBRUBROS.map(s => {
          const sel   = subrubros.includes(s.key)
          const badge = TIPO_BADGE[s.tipo]
          return (
            <button
              key={s.key}
              onClick={() => toggleSubrubro(s.key)}
              style={{
            ...
            >
              ...
              <div style={{ fontSize: 11, color: 'var(--color-muted)', lineHeight: 1.4, marginBottom: badge ? 8 : 0 }}>
                {s.descripcion}
              </div>
              {badge && (
                <div style={{
  ...
  return (
    <div style={{ maxWidth: 540, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 14, color: 'var(--color-muted)', margin: 0 }}>
          Esta información aparecerá en tu tienda online.
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <Field label="Nombre del negocio" required>
          <Input value={negocio.nombre} onChange={set('nombre')} placeholder="Ej: Mi Tienda Online" />
        </Field>
        ...
        {/* Subdominio */}
        <Field label="Subdominio de tu tienda">
          <div style={{
          ...
          <p style={{ fontSize: 11, color: 'var(--color-muted)', margin: '5px 0 0' }}>
            Una vez activa tu tienda, podés conectar un dominio propio como <strong>tunegocio.com.ar</strong>.
          </p>
        </Field>
...
export function TiendaSetup() {
  ...
  function toggleSubrubro(key: string) {
    setSubrubros(prev => {
  ...
  const puedeAvanzar = paso === 0 ? subrubros.length > 0 : true

  function avanzar() {
    if (paso < 2) { setCargandoPaso(true); setPaso(p => p + 1) }
    else router.push('/onboarding/tienda/success')
  }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '36px 24px 160px' }}>
        {cargandoPaso
          ...
          : <>
              {paso === 0 && <StepTipo    subrubros={subrubros} toggleSubrubro={toggleSubrubro} />}
      ...
      <OrbiChat
        ...
        conBarra
        mensaje="¿Tenés alguna duda al configurar tu tienda? Estoy acá para ayudarte."
        quickActions={[
